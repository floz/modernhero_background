...
Main = (function() {
  ...
  Main.prototype._showLines = function() {
    var idx, line, speed, _i, _len, _ref;
    this._speeds = [8, 4, 3, 2.8, 2.7, 2.5, 2.3];
    this._delays = [0, 2, 3, 3.5, 3.75, 3.9, 4.1];
    idx = 0;
    speed = 1;
    _ref = this._$lines;
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      line = _ref[_i];
      TweenLite.to(line, this._speeds[idx] * .5, {
        height: this._heights[idx] + "px",
        delay: this._delays[idx],
        ease: Quad.easeOut
      });
      idx++;
    }
    TweenLite.to(this._$background_sky, 5.5, {
      bottom: 0
    });
    return TweenLite.to(this._$background_ground, 1, {
      bottom: 0,
      delay: 4.4,
      ease: Quad.easeOut
    });
